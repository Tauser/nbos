...
constexpr int16_t EyeDirtyPadding = 16;
...
DirtyRect compute_eye_dirty_rect(const ncos::services::face::FaceFrame& previous,
  ...
  DirtyRect rect{};
  const int16_t unclipped_x0 = x0;
  const int16_t unclipped_y0 = y0;
  const int16_t unclipped_x1 = x1;
  const int16_t unclipped_y1 = y1;
  ...
  rect.clipped = unclipped_x0 != x0 || unclipped_y0 != y0 || unclipped_x1 != x1 ||
                 unclipped_y1 != y1;
  rect.valid = x1 > x0 && y1 > y0;
  rect.x = x0;
  rect.y = y0;
  rect.w = rect.valid ? static_cast<int16_t>(x1 - x0) : 0;
  rect.h = rect.valid ? static_cast<int16_t>(y1 - y0) : 0;
  return rect;
}
...
DisplayRenderPlan analyze_render_plan(const ncos::services::face::FaceFrame* previous,
  ...
  plan.dirty_rect = compute_eye_dirty_rect(*previous, current, display_width, display_height);
